...
if (!class_exists('setModuleGridBodyCls')) {
    ...
    class setModuleGridBodyCls extends Application_Maker_setField {
        ...
        public function setPostsStatus() {
            global $current_user, $post_id, $oThis, $post, $meta_marker ,$main_config;
			
			$tmp_user = false;
			if($main_config['other_roles'] != ''){
				foreach(explode(',',$main_config['other_roles']) as $val){
					if($current_user->roles[0] == $val)
						$tmp_user = true;
				}
			}
            if (current_user_can('administrator') or current_user_can('editor') or $tmp_user) {
                $args = $_POST['args'];
